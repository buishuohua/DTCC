...
class GRN(nn.Module):
    def __init__(self, d_model, context_size=None, dropout=0.1):
        super().__init__()
        ...
        self.W2 = nn.Linear(self.d_model, self.d_model, bias=True)  # W₂,ω for input a
        if self.context_size > 0:
            self.W3 = nn.Linear(self.context_size, self.d_model, bias=False)  # W₃,ω for context c

        # η₁ = W₁,ω η₂ + b₁,ω
        self.W1 = nn.Linear(self.d_model, self.d_model)  # W₁,ω

        self.dropout = nn.Dropout(dropout)
        self.elu = nn.ELU()
        self.glu = GLU(self.d_model)
